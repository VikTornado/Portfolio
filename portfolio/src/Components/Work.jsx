...
const Work = (props) => {
    return (
        <div id={"work"} className={'max-w-[1040px] m-auto md:pl-14 py-16'}>
            <h1 className={'text-4xl font-bold text-center pb-4 text-[#001b5e]'}>Work</h1>
            {props.data.map((item) => {
                return (
                    <ol className={'flex flex-col md:flex-row relative border-l border-stone-200 ml-14'}>
                        <li className={'mb-10 ml-4'}>
                            ...
                            <p className={'flex flex-wrap px-2 gap-4 flex-row items-center justify-start text-xs md:text-sm'}>
                                <span
                                    className={'inline-block px-2 py-1 font-semi-bold text-white bg-violet-900 rounded-md'}>{item.year}</span>
                                <span className={'text-lg font-semi-bold text-violet-900'}>{item.title}</span>
                                <span
                                    className={'my-1 text-sm font-normal leading-none text-stone-400'}>{item.duration}</span>
                            </p>
